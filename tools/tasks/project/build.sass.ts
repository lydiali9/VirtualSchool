...
const designSCSSFiles   = join(Config.DESIGN_ASSETS_SRC, '**', '*.scss');
...
/**
  * Process DESIGN SCSS Files beginning with app.scss and moving down the hierarchy.
  */
function processDesignScss() {
  return gulp.src(join(Config.DESIGN_ASSETS_SRC, 'scss', 'app.scss'))
    .pipe(sassGlob())
    .pipe(plugins.sourcemaps.init())
    .pipe(plugins.sass(Config.getPluginConfig('design-sass')).on('error', plugins.sass.logError))
    .pipe(plugins.postcss(processors))
    .on('error', reportPostCssError)
    .pipe(plugins.sourcemaps.write())
    .pipe(gulp.dest(isProd ? Config.TMP_DIR : Config.CSS_DEST));
}
 ...
 */
export =
  class BuildCSS extends CssTask {
    run() {
      return merge(processComponentStylesheets(), processExternalStylesheets(), processDesignScss());
    }
  };
